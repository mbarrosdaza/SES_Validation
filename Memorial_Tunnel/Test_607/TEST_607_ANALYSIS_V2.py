...
df = pd.read_csv('./Measured_data/HRR607.csv', skiprows=[1])
...
subset = df.iloc[28:32, 1:3].astype(float)
...
print("Minimum HRR from iloc[22:51]:", min_value)
print("Average HRR from iloc[22:51]:", average_hrr)
print("Maximum HRR from iloc[22:51]:", max_value)
...
T607_measured_data_Temp = T607_measured_data_Temp.sort_values(by=['distance_m'])
...
files = [
    './SES_results/MT-T607-R3-1.xlsx',   # Min HRR
    './SES_results/MT-T607-R3-2.xlsx',   # Min HRR (different variation)
    './SES_results/MT-T607-R3.xlsx',     # Average HRR
    './SES_results/MT-T607-R3-3.xlsx',   # Average HRR with variation
    './SES_results/MT-T607-R3-4.xlsx',   # Max HRR
    './SES_results/MT-T607-R3-5.xlsx'    # Max HRR (different variation)
]
...
for file in files:
    ...
    T607_Temp = T607_Temp.astype(float)
    densi_upstream_607 = pressure / (R_constant * (273.15 + T607_Temp.iloc[0, 1:].mean()))
    flow2 = [f * densi_upstream_607 / dens for f, dens in itertools.zip_longest(flow, density)]
...
plt.text(0.95, 0.7, 'Downstream', fontsize=8, transform=plt.gca().transAxes, 
...
for i, df in enumerate(ses_results_all):
    ...
    if i == 2:  # Average HRR (file index 2)
        ...
        T607_Temp_avg_upstream = df.loc[df['length'] < 605, 'temperature'].mean()
        print('predicted avg temperature upstream the fire in T607' + labels[a], T607_Temp_avg_upstream)
        T607_Temp_avg_downstream = df.loc[df['length'] > 605, 'temperature'].mean()
        print('predicted avg temperature downstream the fire in T607' + labels[a], T607_Temp_avg_downstream)
...
plt.savefig('Test_607_Temperature_Distribution.png', dpi=300)
...
for i, df in enumerate(ses_results_all):
    ...
    if i == 2:  # Average HRR (file index 2)
        ...
        T607_Flow_avg_upstream = df.loc[df['length'] < 605, 'flowrate'].mean()
        print('predicted avg flow upstream the fire in T607' + labels[a], T607_Flow_avg_upstream)
        T607_Flow_avg_downstream = df.loc[df['length'] > 605, 'flowrate'].mean()
        print('predicted avg Flow downstream the fire in T607' + labels[a], T607_Flow_avg_downstream)
    # Highlight the average HRR with variation result with a distinct style (green with square marker)
    elif i == 3:  # Average HRR with variation (file index 3)
...
# Save the plot with high resolution (DPI = 300)
plt.savefig('Test_607_Flowrate_Distribution.png', dpi=300)
...
plt.savefig('Test_607_HRR.png', dpi=300)
...
tab_names = ['MT-T607-R3-1.xlsx','MT-T607-R3-2.xlsx', 'MT-T607-R3.xlsx', 'MT-T607-R3-3.xlsx', 'MT-T607-R3-4.xlsx', 'MT-T607-R3-5.xlsx' ]

with pd.ExcelWriter('Outputs/SES_results_607.xlsx', engine='openpyxl') as writer:
