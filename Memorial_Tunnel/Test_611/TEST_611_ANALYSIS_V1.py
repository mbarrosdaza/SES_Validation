...
T611_HRR = T611_HRR_data.iloc[:, [1,2]].astype(
...
T611_measured_data_Temp = T611_measured_data_Temp.sort_values(by=[
                                                              'distance_m'])
plt.figure()
...
file = './SES_results/MT-T611-R3.xlsx'
temp = []
...
densi_upstream_611 = 1.251842913
flow2 = [f * densi_upstream_611/dens for f,
         dens in itertools.zip_longest(flow, density)]
SES_results_611 = pd.DataFrame({"length": SES_segment_length['distance'].tolist(
...
'upstream '
T611_Temp_avg_upstream = SES_results_611.loc[SES_results_611['length'] < 605, 'temperature'].mean()
T611_Temp_avg_upstream_mea = T611_measured_data_Temp.loc[T611_measured_data_Temp['distance_m'] < 605, 'Temp_C'].mean()
print('predicted avg temperature upstream the fire in T611', T611_Temp_avg_upstream)
print('measured avg temperature upstream the fire in T611', T611_Temp_avg_upstream_mea)

'Downstream '
T611_Temp_avg_downstream = SES_results_611.loc[SES_results_611['length'] > 605, 'temperature'].mean()
T611_Temp_avg_downstream_mea = T611_measured_data_Temp[(T611_measured_data_Temp['distance_m'] > 605) ]['Temp_C'].mean()
print('predicted avg temperature downstream the fire in T611', T611_Temp_avg_downstream)
print('measured avg temperature downstream the fire in T611', T611_Temp_avg_downstream_mea)
...
T611_Flow_avg_upstream = SES_results_611.loc[SES_results_611['length'] < 605, 'flowrate'].mean()
T611_Flow_avg_upstream_mea = T611_measured_data_Flow.loc[T611_measured_data_Flow['distance_m'] < 605, 'flow'].mean()
print('predicted avg flow upstream the fire in T611', T611_Flow_avg_upstream)
print('measured avg flow upstream the fire in T611', T611_Flow_avg_upstream_mea)

'Downstream '
T611_Flow_avg_downstream = SES_results_611.loc[SES_results_611['length'] > 605, 'flowrate'].mean()
T611_Flow_avg_downstream_mea = T611_measured_data_Flow[(T611_measured_data_Flow['distance_m'] > 605) ]['flow'].mean()
print('predicted avg Flow downstream the fire in T611', T611_Flow_avg_downstream)
print('measured avg Flow downstream the fire in T611', T611_Flow_avg_downstream_mea)


##############################################################################################

# Calculate average HRR as the mean between 'Based On Efficiency' and 'Raw (Calculated)' 
T611_HRR_data['HRR_avg'] = T611_HRR_data.iloc[:, [1, 2]].astype(float).mean(axis=1)

# Calculate negative flow from 'Loop 209'
avg_flow = -Q611_Flow['Loop 209'].astype(float)
time_flow = Q611_Flow['Time'].astype(float)
time_hrr = T611_HRR_data['Time'].astype(float)
...
hrr_common = T611_HRR_data[T611_HRR_data['Time'].isin(common_time)]['HRR_avg'].reset_index(drop=True)
